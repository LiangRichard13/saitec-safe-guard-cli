...
# ssgc CLI 操作指南（Agent 版）

反向代理监控工具：客户端把大模型 API 请求指向本地端口，ssgc 透明转发到真实上游，同时记录请求/响应并周期上报到检测服务器，检测结果存本地 SQLite。
...
客户端 ──→ 127.0.0.1:<port> ──→ upstream（真实端点）
           ...
           JSONL 落盘 ──周期上报──→ detector（检测服务器）
              ▼
           results.db（SQLite）←── ssgc report 查询
```
...
- **配置路径**：默认 `~/.ssgc/config.json`；用环境变量 `SSGC_CONFIG=/path/config.json` 隔离实例（测试/多套配置必备）。数据（records/logs/results.db）跟随 config 所在目录。
...
```bash
# 生命周期
ssgc init --api-key KEY --detector-url URL --upstream URL [--endpoint-type T] [--name N] [--port P] [--force]
ssgc start [--report-interval N] [--batch-size N]   # 后台子进程 + PID 文件
ssgc monitor [--report-interval N]                  # 前台实时监控（人盯）：异常彩色输出，Ctrl+C 退出
ssgc stop [--timeout N]
ssgc restart
ssgc status --json        # running/pid/services/最近日志
ssgc doctor --json        # 自检（config/端口/磁盘/SQLite/JSONL/api_key）

# 监控服务管理
ssgc service list --json
ssgc service add NAME --upstream URL [--endpoint-type T] [--port N]
ssgc service set NAME [--upstream URL] [--port N] [--endpoint-type T] [--record-body B]
ssgc service remove NAME

# 配置
ssgc config get KEY [--json]          # api_key 自动脱敏
ssgc config set KEY VALUE [--json]    # 原子写入：校验失败不落盘，自动备份
ssgc config unset KEY
...
```bash
export SSGC_CONFIG=$WORK_DIR/config.json
ssgc init --api-key "$DETECTOR_KEY" --detector-url "$DETECTOR_URL" \
...
```bash
ssgc service add local-llm --upstream http://localhost:23333 --port 9010 --json
ssgc service set local-llm --upstream http://localhost:11434 --json
ssgc service remove local-llm --json
ssgc restart --json
```
...
```bash
ssgc report --since 1h --json     # data.results[]: record_id/detection_status/risk_level/model/detail
ssgc report --since 7d --limit 500 --json | jq '[.data.results[] | select(.detection_status=="violation")]'
```
...
```bash
ssgc status --json && ssgc report --since 10m --limit 500 --json
```
...
1. `ssgc doctor --json` —— `port:*` fail = 端口冲突；`config` fail = 配置坏
2. `status` 未运行但怀疑有残留 —— 直接 `start`：CLI 对死 PID 自动覆盖（stop 也自带 STALE_PID 自愈清理）。真正报 `ALREADY_RUNNING` 说明 PID 确实活着，是另一个实例在跑（可能用了不同 SSGC_CONFIG），先找到它再决定停谁
...
| 改配置没生效 | `restart` |
| 监控需要真实模型 key | ssgc 不做鉴权，`Authorization` 头由客户端自带透传；测试用 mock（见 references） |
